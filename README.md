...
Установленная IDE обновляется сама: exe — тонкий «загрузчик» (`bootstrap.py`),
а логика (`server.py`, `index.html`, `vendor/`) — обычные файлы рядом. Фоновый
поток раз в несколько часов проверяет последний релиз на GitHub (по редиректу
`releases/latest`, без API-лимитов), скачивает лёгкий `payload-<версия>.zip`
(~0.5 МБ), сверяет sha256 и складывает в `updates/pending`. Загрузчик применяет
его при следующем запуске — с бэкапом и автооткатом, если новая версия не
стартовала. В статус-баре появляется тихая подсказка «обновление готово —
перезапустите». Офлайн-машины просто ничего не замечают.

Правила выпуска:
- `python build.py` теперь кладёт в `installer-out/` **три** артефакта:
  `UNI-IDE-Setup-<v>.exe`, `payload-<v>.zip`, `payload-<v>.zip.sha256`.
  В релиз GitHub загружаются все три (payload обязателен — иначе установленные
  копии предложат ученикам скачивать полный установщик).
- Если менялись `bootstrap.py` или python-зависимости (flask/pyserial/pywebview) —
  поднимите `PAYLOAD_MIN_BASE` в `build.py` до новой версии: старые установки
  увидят плашку «нужен полный установщик» вместо кривого payload-обновления.
- Новый import в `server.py` должен быть в списке предзагрузки `bootstrap.py`
  (иначе замороженный python его не найдёт) — это тоже повод поднять
  `PAYLOAD_MIN_BASE`.
