# UNI IDE — простая IDE для ESP32 Dev Module
...
**ESP32 Dev Module** (`FQBN = esp32:esp32:esp32`).
...
5. **◷ Монитор** — serial-монитор (выбор скорости, отправка строк в порт).
...
Изменения в коде копятся обычными коммитами. Когда нужно выпустить версию:

1. Поднимите номер версии в `build.py` (`VERSION = "1.0.0"`).
2. Соберите установщик: `python build.py` → `installer-out\UNI-IDE-Setup-<версия>.exe`.
3. Создайте релиз на GitHub и приложите установщик. Сам `.exe` (сотни МБ) **не
   коммитится** в репозиторий — он раздаётся через GitHub Releases:
   ```
   gh release create v1.0.0 "installer-out\UNI-IDE-Setup-1.0.0.exe" --title "UNI IDE 1.0.0" --notes "Что нового"
   ```

Ссылка «Скачать» в начале README ведёт на `/releases/latest` — её менять не нужно,
она всегда указывает на самый свежий релиз.

---

## Подключение библиотеки UniBaseControl

Вашу библиотеку из проекта можно положить в папку скетча рядом с `.ino`
(будет видна как `#include "UniBaseControl.h"`) или установить как обычную
Arduino-библиотеку через `arduino-cli lib install --git-url <repo>` и
подключать `#include <UniBaseControl.h>`.

## Что дальше (идеи для развития)

- Свой «быстрый» `bootloader.bin` без логирования/CRC и прямой вызов `esptool`
  для мгновенного старта (как в исходной заметке).
- Визуальный редактор блоков (ArduBlock-стиль) → генерация C++.
- Пресеты насадок UNI (maze / line / dancer / grip) как шаблоны скетчей.
- Упаковка в один исполняемый файл через PyInstaller, чтобы не ставить Python.
